...
void guassian_blur(int N, double *p_blur, double *p, double *p_blur_h, double sigma){
	...
	/*double p_blur_total = 0.0f;
	double p_total = 0.0f;
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= N; j++) {
			p_blur_total = p_blur[IX(i, j)];
			p_total += p[IX(i, j)];
		}
	}
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= N; j++) {
			p_blur[IX(i, j)] *= p_total / p_blur_total;
		}
	}*/
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= N; j++) {
			p_blur[IX(i, j)] /= sqrt(2 * M_PI * sigma * sigma);
		}
	}
...
void add_driving_force(int N, double *u, double *v, double *u_prev, double *v_prev, double *p_blur, double *p_target_blur, double vf, double dt) {
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= N; j++) {
			double gradx = 0.5f * N * (p_target_blur[IX(i + 1, j)] - p_target_blur[IX(i - 1, j)]);
			double grady = 0.5f * N * (p_target_blur[IX(i, j + 1)] - p_target_blur[IX(i, j - 1)]);
			if (p_target_blur[IX(i, j)] == 0.0f) {
				u[IX(i, j)] = u_prev[IX(i, j)];
			}
			else {
				u[IX(i, j)] = u_prev[IX(i, j)] + dt * vf * gradx * p_blur[IX(i, j)] / p_target_blur[IX(i, j)];
			}
			if (p_target_blur[IX(i, j)] == 0.0f) {
				v[IX(i, j)] = v_prev[IX(i, j)];
			}
			else {
				v[IX(i, j)] = v_prev[IX(i, j)] + dt * vf * grady * p_blur[IX(i, j)] / p_target_blur[IX(i, j)];
			}
		}
	}
	set_boundary(N, 1, u);
...
void gather(int N, double *p, double *p_prev, double *p_target, double *p_target_blur, 
	...
	for (int i = 1; i <= N; i++) {
		for (int j = 1; j <= N; j++) {
			...
			double divergence = 0.5 * (x1 - x0 + y1 - y0) / N;
